...
def get_args():
    ...
    parser.add_argument("--arch", type=str, default="ImageNet_arch", help="which architecture to use")
...
args.gpu_num = 5                           # torch.cuda.device_count()
...
def main():
    ...
    #train_queue, valid_queue, sampler = utils._get_imagenet(args)               # utils._get_dist_imagenet(args)
    logging.info(f'rank {args.local_rank}: load data successfully')

    genotype = eval("genotypes.%s" % args.arch)
    model = Network(args.init_channels, args.class_num, args.layers, args.auxiliary, genotype).to(device)
    model = nn.DataParallel(model)

    criterion = nn.CrossEntropyLoss().to(device)
    criterion_smooth = utils.CrossEntropyLabelSmooth(args.class_num, args.label_smooth).to(device)
